...
import { classNames, get_business_report_bureau } from "~/utils/helpers";
import CacheLink from "./CacheLink";

export const VerticalNav = ({ selected = "Personal", report_plan_id = "essential" }) => {
	let location = useLocation();
	let search_params = location.search;
	let business_bureau = get_business_report_bureau(location.pathname);

	return (
		<nav className="flex flex-col justify-star bg-white rounded" aria-label="Tabs">
			{pipe(
				...
				map((tab) => (
					...
					>
						<div className="flex flex-row justify-between w-full">
							...
							{selected == tab.id && pipe(includes(report_plan_id), not)(tab.plans) && (
								<div>
