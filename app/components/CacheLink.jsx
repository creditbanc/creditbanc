...
import { filter, includes, isEmpty, join, keys, map, pipe, __, head } from "ramda";
...
export default function CacheLink({ to, children }) {
	...
	const create_cache_string = (path, cache) => {
		let get_route_key = pipe(get("routes"), keys, filter(includes(__, path)), head);
	...
	useEffect(() => {
		if (!isEmpty(cache.keys)) {
			...
			let link = to.includes("?") ? `${to}&${cache_string}` : `${to}?${cache_string}`;
			setHref(link);
		}
	}, [cache.keys]);
