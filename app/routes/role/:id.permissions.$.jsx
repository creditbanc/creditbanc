...
import { classNames, get_config_id, get_group_id, mapIndexed, trim, use_client_search_params } from "~/utils/helpers";
...
export const usePermissionsStore = create((set) => ({
	permissions: [],
	set_permissions: (path, value) => set((state) => pipe(mod(...path)(() => value))(state)),
}));
...
const SectionHeading = () => {
	return (
		<div className="flex flex-row justify-between items-end w-full border-b border-gray-200 pb-5">
			<div>
				<h3 className="text-base font-semibold leading-6 text-gray-900">Pages Permissions</h3>
				<p className="mt-2 max-w-4xl text-sm text-gray-500">Select what pages can be viewed or edited.</p>
			</div>
			{/* <div>
	...
	return (
		<div className=" pb-2">
			<div className="-ml-2 -mt-2 flex flex-wrap items-baseline">
				<h3 className="ml-2 mt-2 text-base font-semibold leading-6 text-gray-900">Role:</h3>
				<h3 className="ml-1 mt-1 truncate text-base text-gray-500">{config?.name}</h3>
			</div>
		</div>
	);
};
				...
				<div className="flex flex-col w-full divide-y">
					{pipe(
						mapIndexed((permission, index) => (
							<Permission permission={permission} index={index} key={index} />
						))
					)(permissions)}
				</div>
			</div>
