...
export const loader = async ({ request }) => {
	...
	return { credit_utilization, plan_id };
};
			...
			<div className="border-t border-gray-200 p-5 space-y-5">
				<div className="flex flex-col w-full [&>*:nth-child(odd)]:bg-gray-50 border rounded">
					<div className="flex flex-row py-2 px-3">
						...
						<div>{data?.averageHighCreditAmount || 0}</div>
					</div>
					<div className="flex flex-row py-2 px-3">
						<div className="flex flex-col w-3/4">
							Maximum High Credit Amount
						</div>
						<div>{data?.maximumHighCreditAmount || 0}</div>
					</div>
					<div className="flex flex-row py-2 px-3">
						<div className="flex flex-col w-3/4">
							High Credit Experiences Count
						</div>
						<div>{data?.highCreditExperiencesCount || 0}</div>
					</div>
					<div className="flex flex-row py-2 px-3">
						<div className="flex flex-col w-3/4">
							Satisfactory Experiences Count
						</div>
						<div>{data?.satisfactoryExperiencesCount || 0}</div>
					</div>
					<div className="flex flex-row py-2 px-3">
						<div className="flex flex-col w-3/4">
							Satisfactory Experiences Amount
						</div>
						<div>{data?.satisfactoryExperiencesAmount || 0}</div>
					</div>
					<div className="flex flex-row py-2 px-3">
						<div className="flex flex-col w-3/4">
