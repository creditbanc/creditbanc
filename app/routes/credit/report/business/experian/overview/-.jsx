import { HandThumbUpIcon, ChevronDoubleRightIcon } from "@heroicons/react/24/outline";
...
const credit_report = subject.pipe(
	...
	concatMap(({ args: { request } }) => {
		...
		let result = report.business_info.experian_score.experian_risk_class.experian_trade_summary.fold;

		return result.pipe(
			tap(() => console.log("credit.report.business.experian.overview.tap")),
			tap(inspect)
		);
		...
		// 	concatMap(({ entity_id, group_id }) =>
		// 		throwError(() =>
		// 			Response.redirect(`${url.origin}/credit/business/new/resource/e/${entity_id}/g/${group_id}`)
		// 		)
		// 	)
		// );
		...
		// let experian_score = $report.pipe(rxmap((report) => report.experian_score()));

		// let experian_risk_class = $report.pipe(rxmap((report) => report.experian_risk_class()));

		// let business_info = $report.pipe(rxmap((report) => report.business_info()));

		// let experian_trade_summary = $report.pipe(rxmap((report) => report.experian_trade_summary()));
