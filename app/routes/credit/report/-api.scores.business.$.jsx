...
import { get_file_id, get_group_id, inspect, get } from "~/utils/helpers";
...
const log_route = `credit.report.api.scores.business`;
...
const credit_scores = subject.pipe(
	...
	concatMap(({ args: { request } }) => {
		...
		return business_report.scores.fold;
		...
		// let report_id = db_report.pipe(
		// 	rxfilter((value) => value !== undefined),
		// 	rxmap(pipe(get("id")))
		// );

		// let application_id = db_report.pipe(rxmap(pipe(get("application_id"))));
		...
		// 	rxfilter((value) => value === undefined),
		// 	rxmap(() => ({ dnb_score: () => 0, experian_score: () => 0, is_empty: () => true }))
		// );
...
export const loader = async ({ request }) => {
	...
	let group_id = get_group_id(url.pathname);
	let business_report = new BusinessReport(group_id);
	let result = await lastValueFrom(business_report.scores.fold.pipe(fold(on_success, on_error)));
	return result;
};
