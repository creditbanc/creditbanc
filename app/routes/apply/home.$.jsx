...
export const action = async ({ request }) => {
	...
	let step = pipe(filter({ id: "home" }), head, get("step"))(navigation);
	let { homeownership } = params;
	let payload = { homeownership };
	...
	let next = pipe(filter({ id: "home" }), head, get("next"))(navigation);
						...
						<div className="col-span-full">
							...
							<div className="mt-2">
								<input
									...
									onChange={(e) => {
										set_path(["homeownership"], e.target.value);
									}}
								/>
							</div>
						</div>
...
export default function Container() {
	...
	let back = pipe(filter({ id: "home" }), head, get("back"))(navigation);
	...
	const onSubmit = () => {
		...
		submit(payload, {
			action: `/apply/home/resource/e/${entity_id}/g/${group_id}`,
			method: "post",
		});
	};
